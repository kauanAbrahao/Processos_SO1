...
public class RedesController {
	...
	public void IP(String so) {
		//Recebe o nome do S.O. como par�metro e faz a chamada de configura��o de IP e filtra a sa�da do processo, retornando
		...
		if (so.contains("Windows")) {
			...
			try {
				Process p = Runtime.getRuntime().exec("ipconfig"); //<cmd.exe /c start cmd.exe> se eu quiser de fato abrir o cmd, n�o s� executar
				...
				if (msgErro.contains("error=740")) {
					...
					try {
						...
						BufferedReader buffer2 = new BufferedReader(leitor2); //essas linhas s�o padr�es para leitura de processos.
			...
			if (so.contains("Linux")) {
				...
				System.out.println("N�o foi poss�vel executar por conta da imcompatibilidade de sistema! ");
	...
	public void PING(String so) {
		//Recebe o nome do S.O. como par�metro e faz a chamada de ping com 10 itera��es, pegando e devolvendo, em ms, apenas o tempo m�dio.
				...
				try {
					...
					while (linha != null) {
						if (linha.contains("avg")) {
							...
							System.out.println("o tempo médio de ping para www.google.com.br foi de: " + ms[4] + "ms" );
						}
						linha = buffer.readLine();
					}
